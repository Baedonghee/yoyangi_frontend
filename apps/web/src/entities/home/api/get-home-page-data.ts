...
function toInternalHref(url: string) {
  try {
    ...
    if (
      /yoyangi\.com$/i.test(parsedUrl.hostname) ||
      /yoyangi\.co\.kr$/i.test(parsedUrl.hostname)
    ) {
...
function findBannerImage(
  ...
  return images.find((image) => matcher(image.type.trim().toUpperCase()))
    ?.imageUrl;
}
...
function mapBanners(list: BannerApiItem[]): HeroBanner[] {
  return list.map((banner, index) => {
    ...
    return {
      ...
      title:
        banner.title ||
        mockHomePageData.heroBanners[index]?.title ||
        "요양이 메인 배너",
      description:
        banner.description ||
...
function mapYoutubeItems(
  list: YoutubeApiItem[],
  meta: string,
): HomePageData["tvItems"] {
  return list
...
export async function getHomePageData(): Promise<HomePageData> {
  const [bannerResponse, youtubeResponse, careYoutubeResponse, regionResponse] =
    await Promise.all([
      fetchServerApi<ApiListResponse<BannerApiItem[]>>(
        "/api/v1/app/banners?location=L1",
        {
          debugLabel: "banners",
          fallbackData: {
            status: 500,
      ...
      fetchServerApi<ApiListResponse<YoutubeApiItem[]>>(
        "/api/v1/care-youtubes",
        {
          debugLabel: "care-youtubes",
          fallbackData: {
            status: 500,
            message: "fallback",
            list: [],
  ...
  const careYoutubes = unwrapApiList(careYoutubeResponse, []);
  const regions = unwrapApiList(regionResponse, []);

  const heroBanners = banners.length
    ? mapBanners(banners)
    : mockHomePageData.heroBanners;
  const tvItems = youtubes.length
    ? mapYoutubeItems(youtubes, "요양이 TV")
    : mockHomePageData.tvItems;
  const careYoutubeItems = careYoutubes.length
    ? mapYoutubeItems(careYoutubes, "요양여지도")
    : mockHomePageData.careYoutubeItems;
...
export async function getRecommendedFacilities(): Promise<
  RecommendedFacility[]
> {
